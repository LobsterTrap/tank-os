...
**Goal:** Answer Open Questions 1, 6, and 7 from `docs/dev/csb-bootc-deployment-design.md` by hand-running redhat-et/openclaw-csb's published image inside an OpenShell sandbox on a real tank-os VM — before committing to rewriting `bootstrap-openshell-sandbox`, the Quadlet units, or `sync-podman-secrets`.
